      ...
      <main>
        ...
        <section className="border-b border-border py-16 md:py-24">
          <div className="mx-auto max-w-6xl px-6">
            <h1 className="mb-4 text-4xl font-bold tracking-tight text-foreground md:text-5xl">Content</h1>
            <p className="max-w-2xl text-lg text-muted-foreground">
              Dev logs, coding sessions, gaming setups, and indie hacking behind the scenes. Follow along as I build in
              public and share everything I learn.
            </p>
          </div>
        </section>
        ...
        <section className="border-b border-border py-16">
          <div className="mx-auto max-w-6xl px-6">
            <div className="mb-8">
              <h2 className="text-2xl font-bold text-foreground">Featured Series</h2>
              <p className="mt-1 text-muted-foreground">Binge-worthy playlists to level up your skills</p>
            </div>
                ...
                >
                  <div className="relative aspect-[16/9] overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6">
                      <h3 className="mb-2 text-xl font-bold text-white">{series.name}</h3>
                      <p className="text-sm text-white/80 line-clamp-2">{series.description}</p>
                    </div>
              ...
              <div className="flex flex-wrap gap-2">
                {platforms.map((platform) => {
                  ...
                  return (
                    ...
                    >
                      {platform !== "All" && <i className={`fa-brands ${platformIcons[platform]}`} aria-hidden="true" />}
                      {platform}
                      <span
                        className={cn(
          ...
          <div className="mx-auto max-w-6xl px-6">
            {filteredContent.length > 0 ? (
                  ...
                  <div className="relative aspect-video lg:aspect-auto lg:h-full min-h-[300px]">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6">
                      <div className="mb-3 flex items-center gap-2">
                        <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium text-white ${platformColors[filteredContent[0].platform]}`}>
                          <i className={`fa-brands ${platformIcons[filteredContent[0].platform]}`} aria-hidden="true" />
                      ...
                      <h3 className="mb-2 text-2xl font-bold text-white">{filteredContent[0].title}</h3>
                      <p className="mb-3 text-sm text-white/80 line-clamp-2">{filteredContent[0].description}</p>
                    ...
                    <div className="p-4">
                      <div className="mb-2 flex items-center gap-2">
                        <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-secondary/50 px-2 py-0.5 text-xs font-medium text-secondary-foreground">
                          <i className={`fa-brands ${platformIcons[item.platform]} text-muted-foreground`} aria-hidden="true" />
                          {item.platform}
                        </span>
              ...
              <div className="py-16 text-center">
                <i className="fa-solid fa-video mb-4 text-4xl text-muted-foreground/50" aria-hidden="true" />
                <p className="text-muted-foreground">No content found for this platform.</p>
              </div>
            )}
          </div>
        </section>
        ...
        <section className="border-t border-border bg-primary/5 py-16">
          <div className="mx-auto max-w-2xl px-6 text-center">
            <h2 className="mb-4 text-2xl font-bold text-foreground">Never miss a video</h2>
            <p className="mb-8 text-muted-foreground">
              Subscribe to my channels to get notified when I drop new dev logs, tutorials, and behind-the-scenes
              content.
            </p>
