...
const useCountUp = (end: number, duration = 2000, start = 0, inView = false) => {
  ...
  return (
    <div ref={ref} className="mb-12 grid gap-6 md:grid-cols-3">
      ...
      <div className="rounded-2xl border border-border bg-card p-8 text-center">
        <p className="mb-2 text-5xl font-bold text-foreground">{impressions.toFixed(1)}M+</p>
        <p className="text-muted-foreground">Monthly Impressions</p>
      </div>
      <div className="rounded-2xl border border-border bg-card p-8 text-center">
        <p className="mb-2 text-5xl font-bold text-foreground">{engagement.toFixed(1)}%</p>
...
const audienceData = {
  ...
  contentThemes: ["Software Development", "Indie Hacking", "Tech Reviews", "Gaming Setups", "Productivity"],
}
...
const collaborationFormats = [
  ...
  },
  {
    ...
    description: "Honest, in-depth reviews of developer tools, software, hardware, and tech products.",
    icon: "fa-star",
  },
  {
    title: "Speaking & Workshops",
    description: "Virtual or in-person talks about indie hacking, React Native, content creation, or tech careers.",
    icon: "fa-microphone",
  },
]
    ...
    <div className="min-h-screen">
      ...
      <main>
          ...
          <div className="mx-auto max-w-6xl px-6">
            <div className="flex items-center gap-3 mb-4">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10">
                <i className="fa-solid fa-briefcase text-primary" aria-hidden="true" />
              </div>
              <span className="text-sm font-medium uppercase tracking-wider text-primary">Media Kit</span>
            </div>
            <h1 className="mb-4 text-4xl font-bold tracking-tight text-foreground md:text-5xl">Stats & Partnerships</h1>
            <p className="max-w-2xl text-lg text-muted-foreground">
              For brands and partners. Here&apos;s who I reach, my content themes, and how we can work together to
        ...
        <section className="border-b border-border bg-secondary/30 py-16">
          <div className="mx-auto max-w-6xl px-6">
            <h2 className="mb-8 text-2xl font-bold text-foreground">Audience Snapshot</h2>
            ...
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="rounded-xl border border-border bg-card p-6">
                <h3 className="mb-4 font-semibold text-foreground">Age Demographics</h3>
                <div className="space-y-3">
                  {audienceData.demographics.map((demo) => (
                    <div key={demo.label}>
                      <div className="mb-1 flex justify-between text-sm">
                        <span className="text-muted-foreground">{demo.label}</span>
                        <span className="font-medium text-foreground">{demo.percentage}%</span>
                      </div>
                      <div className="h-2 overflow-hidden rounded-full bg-secondary">
                        <div className="h-full rounded-full bg-primary" style={{ width: `${demo.percentage}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Top Locations */}
              <div className="rounded-xl border border-border bg-card p-6">
                <h3 className="mb-4 font-semibold text-foreground">Top Locations</h3>
              ...
              {/* Content Themes */}
              <div className="rounded-xl border border-border bg-card p-6">
                <h3 className="mb-4 font-semibold text-foreground">Content Themes</h3>
                <div className="flex flex-wrap gap-2">
                  {audienceData.contentThemes.map((theme) => (
                    <span key={theme} className="rounded-full bg-primary/10 px-3 py-1 text-sm font-medium text-primary">
                      {theme}
                    </span>
                  ))}
                </div>
        ...
        <section className="py-16 md:py-24">
          <div className="mx-auto max-w-6xl px-6">
            <h2 className="mb-8 text-2xl font-bold text-foreground">Platform Breakdown</h2>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    <th className="pb-4 text-left text-sm font-semibold text-foreground">Platform</th>
                    <th className="pb-4 text-left text-sm font-semibold text-foreground">Handle</th>
                    <th className="pb-4 text-right text-sm font-semibold text-foreground">Followers</th>
                    <th className="pb-4 text-right text-sm font-semibold text-foreground">Avg Views</th>
                    <th className="pb-4 text-right text-sm font-semibold text-foreground">Engagement</th>
                  </tr>
                </thead>
                <tbody>
                  {platformStats.map((stat) => (
                    <tr key={stat.platform} className="border-b border-border last:border-0">
                      <td className="py-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`flex h-10 w-10 items-center justify-center rounded-lg ${stat.bgColor} ${stat.color}`}
                          >
                            <i className={`fa-brands ${stat.icon}`} aria-hidden="true" />
                          </div>
                          <span className="font-medium text-foreground">{stat.platform}</span>
                        </div>
                      </td>
                      <td className="py-4 text-muted-foreground">{stat.handle}</td>
                      <td className="py-4 text-right font-semibold text-foreground">{stat.followers}</td>
                      <td className="py-4 text-right text-muted-foreground">{stat.avgViews}</td>
        ...
        <section className="border-t border-border bg-secondary/30 py-16">
          <div className="mx-auto max-w-6xl px-6">
            <div className="mb-8 flex items-end justify-between">
              <div>
                <h2 className="text-2xl font-bold text-foreground">Content Examples</h2>
                <p className="mt-1 text-muted-foreground">A glimpse of what I create</p>
              </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="group relative md:col-span-2 lg:row-span-2 overflow-hidden rounded-2xl border border-border bg-card">
                <div className="relative aspect-video lg:aspect-auto lg:h-full min-h-[300px]">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    <h3 className="mb-2 text-2xl font-bold text-white">{contentExamples[0].title}</h3>
                    <p className="text-sm text-white/80">{contentExamples[0].stats}</p>
                  </div>
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity group-hover:opacity-100">
                    <div className="flex h-16 w-16 items-center justify-center rounded-full bg-white/90 shadow-xl">
                      <i className="fa-solid fa-play ml-1 text-xl text-foreground" aria-hidden="true" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              {contentExamples.slice(1).map((example) => {
                const platformIcon = example.platform === "YouTube" ? "fa-youtube" : example.platform === "TikTok" ? "fa-tiktok" : "fa-instagram"
                return (
                  <div key={example.title} className="group overflow-hidden rounded-2xl border border-border bg-card">
                    <div className="relative aspect-video overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 transition-opacity group-hover:opacity-100">
                        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white/90 shadow-lg">
                          <i className="fa-solid fa-play ml-0.5 text-foreground" aria-hidden="true" />
                        </div>
                      </div>
                    </div>
                    <div className="p-4">
                      <div className="mb-2 flex items-center gap-2">
                        <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-secondary/50 px-2 py-0.5 text-xs font-medium text-secondary-foreground">
                          <i className={`fa-brands ${platformIcon} text-muted-foreground`} aria-hidden="true" />
                          {example.platform}
                        </span>
                        <span className="text-xs text-muted-foreground">{example.stats}</span>
                      </div>
        ...
        <section className="py-16 md:py-24">
          <div className="mx-auto max-w-6xl px-6">
            <h2 className="mb-4 text-2xl font-bold text-foreground">Ways to Work Together</h2>
            <p className="mb-8 max-w-2xl text-muted-foreground">
              I&apos;m selective about partnerships to ensure authenticity. Here are the collaboration formats that work
              best for my audience.
            </p>
              ...
              {collaborationFormats.map((format) => (
                ...
                >
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-primary/10">
                    <i className={`fa-solid ${format.icon} text-primary`} aria-hidden="true" />
                  </div>
                  <div>
                    <h3 className="mb-2 font-semibold text-foreground">{format.title}</h3>
        ...
        <section className="border-t border-border bg-primary/5 py-16 md:py-24">
          <div className="mx-auto max-w-2xl px-6 text-center">
            ...
            <p className="mb-8 text-muted-foreground">
              Interested in a partnership? Get in touch to discuss how we can create authentic content that resonates
              with my audience.
            </p>
