...
const AnimatedCounter = ({ value, suffix = "" }: { value: number; suffix?: string }) => {
  ...
  useEffect(() => {
    const unsubscribe = springValue.on("change", (latest) => {
      if (ref.current) {
        const formatted = latest >= 1000 ? `${Math.floor(latest / 1000)}K` : Math.floor(latest).toString()
  ...
  return (
    ...
    >
      ...
      <motion.div style={{ scale }} className="mx-auto max-w-6xl px-6">
        ...
        >
          <h2 className="mb-4 text-3xl font-bold tracking-tight text-foreground md:text-4xl">Creator Stats</h2>
          <p className="mx-auto max-w-2xl text-muted-foreground">
            Building an engaged community across platforms, sharing dev content and indie hacking stories.
          </p>
        </motion.div>
        ...
        >
          ...
          >
            <p className="mb-1 text-sm font-medium uppercase tracking-wider text-primary">Total Reach</p>
            <p className="text-5xl font-bold text-foreground md:text-6xl">
              <AnimatedCounter value={246} suffix="K+" />
            </p>
            <p className="mt-2 text-sm text-muted-foreground">followers across all platforms</p>
          </motion.div>
        </motion.div>
          ...
          {stats.map((stat, index) => (
            ...
            >
              <motion.i
                ...
                aria-hidden="true"
              />
              <p className="mb-1 text-2xl font-bold text-foreground">{stat.followers}</p>
              <p className="mb-2 text-sm text-muted-foreground">{stat.platform}</p>
              <motion.p
                initial={{ opacity: 0 }}
